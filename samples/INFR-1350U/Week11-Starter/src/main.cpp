...
int main() {
	...
	// Push another scope so most memory should be freed *before* we exit the app
	{
		...
		shader->LoadShaderPartFromFile("shaders/vert_terrain.glsl", GL_VERTEX_SHADER);
		shader->LoadShaderPartFromFile("shaders/frag_terrain.glsl", GL_FRAGMENT_SHADER);
		shader->Link(); 
		...
		imGuiCallbacks.push_back([&]() {
			...
			if (ImGui::SliderFloat3("Height Cut Offs", &heightCutOffs[0], 0, 1)) {
				shader->SetUniform("u_heightCutOffs", heightCutOffs);
			} 
		...
		Texture2D::sptr height = Texture2D::LoadFromFile("images/heightMap.jpg");
		...
		waterShader->LoadShaderPartFromFile("shaders/vert_water.glsl", GL_VERTEX_SHADER);
		waterShader->LoadShaderPartFromFile("shaders/frag_water.glsl", GL_FRAGMENT_SHADER);
		waterShader->Link();
		...
		GameObject obj2 = scene->CreateEntity("terrain");
		{
			...
			BehaviourBinding::Bind<SimpleMoveBehaviour>(obj2);
		}         
		GameObject obj3 = scene->CreateEntity("water");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/plane.obj");
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(material1);
			obj3.get<Transform>().SetLocalPosition(0.0f, 0.0f, 1.0f);
			BehaviourBinding::Bind<SimpleMoveBehaviour>(obj3);
		}
		...
		while (!glfwWindowShouldClose(window)) {
			...
			material1->Set("u_Time", time.DeltaTime);
